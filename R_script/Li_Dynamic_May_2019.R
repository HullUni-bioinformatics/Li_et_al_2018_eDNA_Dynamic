...
Dynamic_back <- Dynamic_rs[c(1:6,8,10)]


names(Dynamic_back)

Dynamic_back2 <- reshape(Dynamic_back, idvar= c("SampleID","Pond","DP","Day","Position","Replicate"), timevar= "Species", direction = "wide")

names(Dynamic_back2) <- gsub("Ratio.", "", names(Dynamic_back2))


Dynamic_back2_sample <- Dynamic_back2[which(Dynamic_back2$Position != "Ctrl"),]

write.csv(Dynamic_back2_sample,file = "Dynamic_2018/relative read counts.csv",row.names = FALSE)
...
####pay attetion to change the Threshold in Line 111 to 0
...
Dynamic_rs_control_filter_mean <- aggregate(Reads~Pond+Replicate+Species, data = Dynamic_rs_control_filter, FUN=mean)

Dynamic_rs_control_filter_sd <- aggregate(Reads~Pond+Replicate+Species, data = Dynamic_rs_control_filter, FUN=sd)




Dynamic_rs_control_filter_sd  <- rename(Dynamic_rs_control_filter_sd,c("Reads"="SD"))

Dynamic_rs_control_Stats <- merge(Dynamic_rs_control_filter_mean, Dynamic_rs_control_filter_sd, by=c("Pond","Replicate","Species"))


#rename the control name
Dynamic_rs_control_Stats$Replicate <-mapvalues(Dynamic_rs_control_Stats$Replicate, c("NEG1","NEG2"), 
                                                                                     c("NTC1","NTC2"))


Dynamic_rs_control_Stats$Pond <-mapvalues(Dynamic_rs_control_Stats$Pond, c("E1","E4"), 
                                               c("(A)","(B)"))


ggplot(Dynamic_rs_control_Stats, aes(x = Species, y= Reads, ymin=Reads-SD,ymax=Reads+SD, fill = Replicate))+
  ...
  labs(x="Species", y="Average read counts")+theme_bw()+
  ...
  theme(text=element_text(size=15),axis.text.x = element_text(angle =90, vjust = 0.5, hjust = 1),
        panel.grid.minor=element_blank(),panel.grid.major =element_blank(),strip.background = element_blank(),
        strip.text=element_text(face="bold",hjust = 0))



###FigA5####
ggsave("Control samples_12S_T0.jpeg",path = "Dynamic_2018/Figures/", width = 10, height = 8, units = "in", dpi=500)
...
BCD_E1_wide.std <-decostand(BCD_E1_wide.rownames, "pa")
...
ggsave("FigA6_Bray_stage.jpeg",Bray_stage_final,path = "Dynamic_2018/Figures/",  width = 8, height = 5, units = "in", dpi=500)

ggsave("FigA6_Bray_stage.pdf",Bray_stage_final,path = "Dynamic_2018/Figures/",  width = 8, height = 5, units = "in", dpi=800)
...
add_final <- ggdraw(add)+draw_plot_label(c("(A)","(B)","(C)","(D)","(E)","(F)","(G)","(H)","(I)","(J)"), c(0.08,0.24,0.4,0.56,0.72), c(0.983,0.983,0.983,0.983,0.983,0.536,0.536,0.536,0.536,0.536), size = 11.5,fontface = "plain")
...
Pond_summary <- read.csv(file = "Dynamic_2018/Fish information summary 2016_new.csv")
...
ggsave("FigA1_number.jpeg",path = "Dynamic_2018/Figures/",  width = 8, height = 4.5, units = "in", dpi=800)
...
ggsave("FigA2_biomass.jpeg",path = "Dynamic_2018/Figures/",  width = 8, height = 4.5, units = "in", dpi=800)
...
Pond_summary_Sep <- Pond_summary[which(Pond_summary$Date == 'Sep-16'),]

Pond_summary_Sep$Pond <-mapvalues(Pond_summary_Sep$Pond, c("(A)","(B)"), c("(A1)","(B1)"))

Dynamic_rs_sample_D0 <- Dynamic_rs_sample[which(Dynamic_rs_sample$Day == 'D0'),]




Dynamic_rs_sample_D0_mean <- aggregate(Reads~Pond+DP+Day+Position+Species, data = Dynamic_rs_sample_D0, FUN=mean)

Dynamic_rs_sample_D0_mean_sum <- aggregate(Reads~Pond+Species, data = Dynamic_rs_sample_D0_mean, FUN=sum)


Dynamic_rs_sample_D0_mean_sum <- Dynamic_rs_sample_D0_mean_sum[which(Dynamic_rs_sample_D0_mean_sum$Reads > 0),]


Dynamic_rs_sample_D0_mean_sum$Pond <-mapvalues(Dynamic_rs_sample_D0_mean_sum$Pond, c("E1","E4"), c("(A1)","(B1)"))

#Combine the sequencing reads and fish summary infromation



Correlation_D0 <-merge(Dynamic_rs_sample_D0_mean_sum, Pond_summary_Sep, by=c("Pond","Species"))
...
COR_abu <- ggplot(Correlation_D0, aes(x = Number, y = Reads))+
            ...
            stat_cor(method = "pearson",label.sep = "\n",label.y=2.5,label.x=3.5,size=4,hjust = 0)+
            labs(x= expression(paste("Fish abundance")), y="Total read counts")+theme_bw()+
            theme(text=element_text(size=15),strip.background = element_blank(),strip.text=element_text(face="bold",hjust = 0),
                  panel.grid.minor=element_blank(),panel.grid.major =element_blank())+
            geom_text_repel(aes(label=Species),size=4,fontface ="italic")

Correlation_D0$Pond <-mapvalues(Correlation_D0$Pond, c("(A1)","(B1)"), c("(A2)","(B2)"))

COR_bio <- ggplot(Correlation_D0, aes(x = Biomass, y = Reads))+
            geom_point(stat="identity",size=2.5)+
            facet_wrap(~Pond,ncol =2)+
            scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                          labels = trans_format("log10", math_format(10^.x)))+
            scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                          labels = trans_format("log10", math_format(10^.x)))+
            geom_smooth(formula= y ~ x,method="lm", se=FALSE,lwd=0.5,na.rm=TRUE, colour='black')+
            stat_cor(method = "pearson",label.sep = "\n",label.y=2.5,label.x=2,size=4,hjust = 0)+
            labs(x= expression(paste("Fish biomass (kg)")), y="Total read counts")+theme_bw()+
            theme(text=element_text(size=15),strip.background = element_blank(),strip.text=element_text(face="bold",hjust = 0),
                  panel.grid.minor=element_blank(),panel.grid.major =element_blank())+
            geom_text_repel(aes(label=Species),size=4,fontface ="italic")


COR <- grid.arrange(COR_abu, COR_bio, ncol = 1, nrow = 2, widths = c(2.7), heights = c(2.5,2.5))


###FigA4_COR#####


ggsave("COR.jpeg",COR,path = "Dynamic_2018/Figures/",  width = 8, height = 9, units = "in", dpi=500)
...
BCD_E1_stage$Pond <- 'E1'

BCD_E4_stage$Pond <- 'E4'


GLMM_BCD <- rbind (BCD_E1_stage,BCD_E4_stage)
...
shapiro.test(transformTukey(GLMM_BCD$Bray))

qqnorm(transformTukey(GLMM_BCD$Bray))




GLMM_BCD.0 <- glm(transformTukey(Bray) ~ Day + Pond, data = GLMM_BCD)

display(GLMM_BCD.0)
summary(GLMM_BCD.0)


GLMM_BCD.2 <-lm(transformTukey(Bray) ~ Day + Pond + Pond:Day,data=GLMM_BCD)


Anova(GLMM_BCD.2, type="II") 


GLMM_BCD.1 <- glmer(transformTukey(Bray) ~  Day + (1 |Pond/Day), family = binomial,
                    data =GLMM_BCD)
display(GLMM_BCD.1)
summary(GLMM_BCD.1)
anova(GLMM_BCD.1)
...
GLMM_BCD.1 <- glmer(transformTukey(Bray) ~  Day + (1 |Pond), family = binomial,
                    data =GLMM_BCD)
display(GLMM_BCD.1)
summary(GLMM_BCD.1)
anova(GLMM_BCD.1)